...
################################################################################
echo -e "#### 04_filter_gatk.sh: Hard-filtering $vcf_in for\n"
echo -e "#### 04_filter_gatk.sh: FisherStrand, RMSMappingQuality, MappingQualityRankSumTest, ReadPosRankSumTest and AlleleBalance ...\n"
vcftools --vcf $vcf_out_soft --remove-filtered-all --max-non-ref-af 0.99 --min-alleles 2 --recode --recode-INFO-all --stdout > $vcf_out_hard
...
nfilt_fd=$(grep -c "FS_gt60" $vcf_out_soft || true)
